...
- 替换 `logo.png`：保留原中文手写感，移除英文副标题，让首页气质更安静。

### 部署

- SW CACHE_NAME: 1.5.8 → 1.5.9，确保预缓存资源里的 logo 更新。

## [1.5.8] - 2026-04-30

### 修复

- 优化窄屏手机上的同步状态条布局，避免“输入码恢复”按钮被意外挤到下一行。
- 将同步条改为两行 grid：第一行显示同步状态和操作按钮，第二行显示修行码。
- “输入码恢复”按钮在窄屏中简化为“恢复”，保持操作仍然可用。

### 部署

- SW CACHE_NAME: 1.5.7 → 1.5.8。
- `index.html` 的 `script.js` / `style.css` query string 升 1.5.7 → 1.5.8。
